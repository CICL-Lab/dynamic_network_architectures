...
class DCN_Conv(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size, extend_scope, morph, if_offset, device):
        ...
        self.bn = nn.InstanceNorm3d(3 * 2 * kernel_size)
...
def main():
    ...
    # conv_layer = Conv(1, 8).to(device)
    input_tensor = torch.randn(2, 1, 32, 32, 32).to(device)  # (batch, channels, depth, height, width)
    # output = conv_layer(input_tensor)
    # print("Conv Output Shape:", output.shape)

    # 测试 DCN_Conv 模块
    print("Testing DCN_Conv module...")
    kernel_size = 3
    extend_scope = 1.0
    morph = 0  # 0: x方向, 1: y方向, 2: z方向
    if_offset = True

    dcn_layer = DCN_Conv(1, 8, kernel_size, extend_scope, morph, if_offset, device).to(device)
    output = dcn_layer(input_tensor)
    print("DCN_Conv Output Shape:", output.shape)
